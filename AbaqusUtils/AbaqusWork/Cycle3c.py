...
odb = session.openOdb(name= 'Y:\FromHPC\MgO\MgO_10MicronD_R1.odb' )
...
frame = odb.steps[ 'Step-3' ].frames[25]
...
for i in range( len(SField['SDV1'].values)  ):
	...
	data12 = SField['SDV200'].values[i].data
	data13 = SField['SDV201'].values[i].data
	data21 = SField['SDV202'].values[i].data
	data22 = SField['SDV203'].values[i].data
	data23 = SField['SDV204'].values[i].data
	data31 = SField['SDV205'].values[i].data
	data32 = SField['SDV206'].values[i].data
	data33 = SField['SDV207'].values[i].data
	
	R=np.array([[data11,data12,data13],[data21,data22,data23],
...
f = open("Cycle3c.vtk", "w")
f.write(out)
f.close()

sessionFrame = session.scratchOdbs['Y:\FromHPC\MgO\MgO_10MicronD_R1.odb'].steps['Session Step'].frames[0]
sessionField = sessionFrame.FieldOutput(name='Field-X', description='Rotation', field=RR)