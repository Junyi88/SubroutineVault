...
odb = session.openOdb(name= 'Y:\FromHPC\MgO\MgO_10MicronD.odb' )
...
SDV2 = TensorFieldOutput(position='element')
...
for i in range( len(SField['SDV1'].values)  ):
	...
	#SDV.add_data(
		...
		#label = curElement)
	data11 = SField['SDV181'].values[i].data
	data12 = SField['SDV182'].values[i].data
	data13 = SField['SDV183'].values[i].data
	data21 = SField['SDV184'].values[i].data
	data22 = SField['SDV185'].values[i].data
	data23 = SField['SDV186'].values[i].data
	data31 = SField['SDV187'].values[i].data
	data32 = SField['SDV188'].values[i].data
	data33 = SField['SDV189'].values[i].data
	
	F=np.array([[data11,data12,data13],[data21,data22,data23],
		[data31,data32,data33]])
	Ft=np.transpose(F)
	C=IDIOT(Ft,F)
	w,v= np.linalg.eig(C)
	vt=np.transpose(v)
	UsI=np.array([[1.0/np.sqrt(w[0]), 0.0, 0.0 ],
		[0.0, 1.0/np.sqrt(w[1]), 0.0],[0.0, 0.0, 1.0/np.sqrt(w[2])]])
	UI=IDIOT(UsI,vt)
	UI=IDIOT(v,UI)
	R=IDIOT(F,UI)
