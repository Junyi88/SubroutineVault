...
for i in range( numNodesTotal ):
	if (dispField [i].instance.name == 'PART-1-1'):
		...
		x[count]=dispField[i].instance.nodes[count].coordinates[0]
		y[count]=dispField[i].instance.nodes[count].coordinates[1]
		z[count]=dispField[i].instance.nodes[count].coordinates[2]
		count=count+1
	print i
