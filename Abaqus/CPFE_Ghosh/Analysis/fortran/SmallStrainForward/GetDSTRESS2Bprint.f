     ...
     1 FCC_MuX,FCC_OhmX,Cubic_MuX,Cubic_OhmX,	   
     2 CinS)
...
C Subroutine to calculate forest parallel and mobile dislocations
      ...
      include 'SLIPSREDUCED.f'
	  
      real*8,intent(in) :: dTIME 
      real*8,intent(in) :: GammaDot(18),DStrain(6),Stress(6)
      real*8,intent(in) :: FCC_MuX(3,3,12),FCC_OhmX(3,3,12)
      real*8,intent(in) :: Cubic_MuX(3,3,6),Cubic_OhmX(3,3,6)
      real*8,intent(out) :: DStress(6)
      real*8 :: FCC_Mu(3,3,12),FCC_Ohm(3,3,12)
      real*8 :: Cubic_Mu(3,3,6),Cubic_Ohm(3,3,6)
      real*8,intent(in) :: CinS(3)
      
      real*8:: HYDROSTRAIN, DGA(18), DUM1
      integer ISLIPS,IX,IY
      ...
      DO IX=1,3
      DO IY=1,3
	     FCC_Mu(IX,IY,ISLIPS)=0.5*(FCC_S(ISLIPS,IX)*FCC_N(ISLIPS,IY)+
     1        FCC_S(ISLIPS,IY)*FCC_N(ISLIPS,IX))
	 
	     FCC_Ohm(IX,IY,ISLIPS)=0.5*(FCC_S(ISLIPS,IX)*FCC_N(ISLIPS,IY)-
     1        FCC_S(ISLIPS,IY)*FCC_N(ISLIPS,IX))
      END DO
      END DO
      END DO
      ...
      DO IY=1,3
	     print *, '----------------------------'
	     print *, 'S=', FCC_S(ISLIPS,IX), FCC_S(ISLIPS,IY)
	     print *, 'N=', FCC_N(ISLIPS,IX), FCC_N(ISLIPS,IY)
	     print *, '(', IX,IY,ISLIPS,')=',FCC_Mu(IX,IY,ISLIPS)
	     print *, '(', IX,IY,ISLIPS,')X=',FCC_MuX(IX,IY,ISLIPS)
      END DO
      END DO
      END DO
      ...
      DO IX=1,3
      DO IY=1,3
	     CUBIC_Mu(IX,IY,ISLIPS)=0.5*(CUBIC_S(ISLIPS,IX)*CUBIC_N(ISLIPS,IY)+
     1        CUBIC_S(ISLIPS,IY)*CUBIC_N(ISLIPS,IX))
	 
	     CUBIC_Ohm(IX,IY,ISLIPS)=0.5*(CUBIC_S(ISLIPS,IX)*CUBIC_N(ISLIPS,IY)
     1        -CUBIC_S(ISLIPS,IY)*CUBIC_N(ISLIPS,IX))
      END DO
      END DO
      END DO
