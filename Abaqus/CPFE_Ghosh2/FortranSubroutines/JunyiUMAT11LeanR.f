...
231   CONTINUE	
      ...
      ! use original co-ordinates X     
      do i =1,9
          kcurlFp(noel,npt,i) = statev(470+I)
      end do
      ...
      call MutexUnlock( 4 )      	
	   ...
	   IF (NOEL.GT.TOTALELEMENTNUM) THEN
	    write(6,*) 'FWORD, NOEL = ', NOEL
	   END IF 	
...
c          kFp(noel,npt,i)= statev(400+i)
		  statev(470+I) = kcurlFp(noel,npt,i) ! PROBLEM 
      END DO
      call MutexUnlock( 3 )   ! unlock Mutex #2		
      ENDIF
c --------------------------------
...
c ------------------------------
c XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
c      call MutexLock( 6 )      ! lock Mutex #1 
      DO i=1,9                                                      
          kFp(noel,npt,i)= statev(400+i)
c		  statev(470+I) = kcurlFp(noel,npt,i) ! PROBLEM 
      END DO
c      call MutexUnlock( 6 )      ! lock Mutex #1 
c --------------------------------------
      DO ISLIPS=1,6
