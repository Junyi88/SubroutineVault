...
C *** USER DEFINED ARRAYS *** Changed SVARS for XFEM
      real*8 :: spin(3,3), tempstrain(3,3) ,stressV(6),dstressinc(6),
     + dtotstran(6),totstran(6),dstressth(6),dstranth(6),
     + SVARS(NSTATV*16), Fdot(3,3), invF(3,3), F(3,3), L(3,3),
     + LETEST1(3,3),LETEST2(3,3),LETESTV(6)
...
C      and stress 
   ...
   !   call KMLT(Fdot,invF,L)
      ...
      END DO
      call MutexUnlock( 3 )      ! lock Mutex #3
...
C    RECOVER stress for calculating residual force
      ...
      call MutexLock( 1 )      ! lock Mutex #1 
      DO i=1,9                                                      
          kFp(noel,npt,i)= statev(86+i)
      END DO
      call MutexUnlock( 1 )      ! lock Mutex #1 
      
      xfo = 0
      
      ! If XFEM is used then extra calculations need to be done
      ! Changes minimised by using a simple offset -xfo- for the
      ! XFEM calculations
      IF (npt == 16) THEN
          xfo = 8
      END IF
      ...
      IF (npt == 8 .OR. npt == 16 ) THEN ! update curl Fp
...
C=======================================================================          
         ...
         DO kint =1,8
             ...
             call MutexLock( 2 )      ! lock Mutex #2 
             DO i=1,3         
                 gausscoords(i,kint) = kgausscoords(noel,kint+xfo,i)                          
             END DO
             call MutexUnlock( 2 )      ! lock Mutex #2 
             call MutexLock( 1 )      ! lock Mutex #1 
             DO i=1,9          
                 svars(86 + i + 126*(kint+xfo-1)) = kFp(noel,kint+xfo,i)         
             END DO
             call MutexUnlock( 1 )      ! lock Mutex #1 
         END DO
...
C   A FULL INTEGRATION GRADIENT SCHEME
      ...
      CALL kcurl(nsvars,svars,knsdv,xnat8,gauss,gausscoords,xfo)
      
      call MutexLock( 3 )      ! lock Mutex #3
      DO kint =1, 8
          DO i=1, 9
              kcurlFp(noel,kint+xfo,i) = svars(37+i + 126*(kint+xfo-1))
          END DO
      END DO
      call MutexUnlock( 3 )      ! lock Mutex #3
