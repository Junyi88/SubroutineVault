...
C *                          Full integration                                   *
C *******************************************************************************
       SUBROUTINE kcurl(nsvars,svars,knsdv,xnat8,gauss,gausscoords,xfo) 
      ...
      !scalars
      integer,intent(in) :: nsvars,knsdv,xfo
...
C
      ...
      fnode(1,kint) = svars((kint+xfo-1)*knsdv+87)
      fnode(2,kint) = svars((kint+xfo-1)*knsdv+88)
      fnode(3,kint) = svars((kint+xfo-1)*knsdv+89)
      end do
C
...
C
      do kint = 1,8
      fnode(1,kint) = svars((kint+xfo-1)*knsdv+90)
      fnode(2,kint) = svars((kint+xfo-1)*knsdv+91)
      fnode(3,kint) = svars((kint+xfo-1)*knsdv+92)
      end do
C
      fmat1 = matmul(fnode,dndx)
...
C
      do kint = 1,8
      fnode(1,kint) = svars((kint+xfo-1)*knsdv+93)
      fnode(2,kint) = svars((kint+xfo-1)*knsdv+94)
      fnode(3,kint) = svars((kint+xfo-1)*knsdv+95)
      end do
C
      fmat1 = matmul(fnode,dndx)
...
!      write(6,*)"z12n"; write(6,fmt8)z12n
      ...
      do kint=1,nnodes
         svars((kint+xfo-1)*knsdv+38) = z11n(kint)
         svars((kint+xfo-1)*knsdv+39) = z12n(kint)
         svars((kint+xfo-1)*knsdv+40) = z13n(kint)
         
         svars((kint+xfo-1)*knsdv+41) = z21n(kint)
         svars((kint+xfo-1)*knsdv+42) = z22n(kint)
         svars((kint+xfo-1)*knsdv+43) = z23n(kint)
         
         svars((kint+xfo-1)*knsdv+44) = z31n(kint)
         svars((kint+xfo-1)*knsdv+45) = z32n(kint)
         svars((kint+xfo-1)*knsdv+46) = z33n(kint)      
      end do !kint 
C     
      RETURN
      END
